...
private:
	...
	void initVertices(std::vector<Vertex>& vertices_)
	{
		...
		for (int i = 0; i < vertices.size()-1; i++)
		{
			...
			glm::vec3 p1 = (vertices[i].position + (normal*0.1f));
			glm::vec3 p2 = (vertices[i].position - (normal * 0.1f));
		...
		glm::vec3 p1 = (vertices[index].position + (normal * 0.1f));
		glm::vec3 p2 = (vertices[index].position - (normal * 0.1f));
...
public:
	curveMaker(std::vector<Vertex> &vertices_) 
	{	
